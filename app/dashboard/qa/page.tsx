...
import { useGetUserQuestions, useUpdateQuestion, useDeleteQuestion } from "@/hooks/qa/useQa";
...
import { PenLine, Trash2, FilterIcon, X, MessageSquare, Tag, AlertCircle, CheckCircle, PlusCircle } from "lucide-react";
...
export default function MyQuestionsPage() {
    ...
    const [questionToToggleStatus, setQuestionToToggleStatus] = useState<{ id: string; newStatus: QuestionStatus } | null>(null);
    ...
    }, [currentPage, questionsPerPage, filterStatus, selectedTagId, sortBy, sortOrder]);

    useEffect(() => {
        setCurrentPage(1);
    }, [filterStatus, selectedTagId, sortBy, sortOrder]);

    const { data: questionData, isLoading, isError, error, refetch, isFetching } = useGetUserQuestions(filterParams);
    ...
    const updateUrlWithFilters = () => {
        ...
        const url = `/dashboard/qa${queryString ? `?${queryString}` : ""}`;
        router.push(url, { scroll: false });
    };

    useEffect(() => {
        updateUrlWithFilters();
    }, [currentPage, filterStatus, selectedTagId, sortBy, sortOrder]);
    ...
    const handleStatusToggle = (question: QaQuestion) => {
        const newStatus: QuestionStatus = question.status === "OPEN" ? "CLOSED" : "OPEN";
        setQuestionToToggleStatus({ id: question.id, newStatus });
        setStatusDialogOpen(true);
    };

    const confirmStatusChange = () => {
        if (questionToToggleStatus) {
            updateQuestion.mutate(
                {
                    id: questionToToggleStatus.id,
                    questionData: { status: questionToToggleStatus.newStatus },
                },
                {
                    onSuccess: () => {
                        setStatusDialogOpen(false);
                        setQuestionToToggleStatus(null);
                    },
                }
            );
        }
    ...
    const getQuestions = () => {
        ...
        if (questionData.questions.length === 0) {
            return (
                <div className="col-span-3 p-8 text-center">
                    ...
                    <p className="text-muted-foreground mb-6">You haven't asked any questions yet or they don't match your filters.</p>
                    <Button asChild>
        ...
        return questionData.questions.map((question) => (
            <Card key={question.id} className="flex flex-col">
                ...
                <CardFooter className="pt-0 flex justify-between border-t px-6 py-3 mt-auto">
                    ...
                    <div className="flex items-center gap-2">
                        ...
                        <Button
                            size="sm"
                            variant="ghost"
                            onClick={() => router.push(`/dashboard/qa/edit/${question.id}`)}
                        >
    ...
    const ActiveFiltersDisplay = () => {
        if (filterStatus === "all" && selectedTagId === "all" && sortBy === "createdAt" && sortOrder === "desc") {
            return null;
        }
        ...
        <div className="container px-0 mx-auto">
            ...
            <Dialog open={statusDialogOpen} onOpenChange={setStatusDialogOpen}>
                <DialogContent>
                    <DialogHeader>
                        <DialogTitle>
                            {questionToToggleStatus?.newStatus === "CLOSED" ? "Close Question" : "Reopen Question"}
                        </DialogTitle>
                        <DialogDescription>
                            {questionToToggleStatus?.newStatus === "CLOSED"
                                ? "Are you sure you want to close this question? Closed questions can still receive answers but are marked as resolved."
                                : "Are you sure you want to reopen this question? This will mark the question as still needing an answer."}
                        </DialogDescription>
                    </DialogHeader>
                    <DialogFooter>
                        ...
                        <Button onClick={confirmStatusChange} disabled={updateQuestion.isPending}>
